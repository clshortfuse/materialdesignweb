...
/**
 * Shared non-expandable row implementation for list items and list options.
 * Concrete components extend this class and register their own element names.
 */
export default CustomElement
  ...
  .expressions({
    ...
    _showStateLayer({ _ariaRole, href, stateLayer }) {
      return stateLayer && (_ariaRole === 'none' || href != null);
    },
    _showRippleContainer({ _ariaRole, disabledState, href }) {
      return !disabledState && (_ariaRole === 'none' || href != null);
    },
    hasSupporting() {
  ...
  .recompose(({
    ...
    state.setAttribute('mdw-if', '{_showStateLayer}');
    state.setAttribute('state-disabled', 'focus hover');
    rippleContainer.setAttribute('mdw-if', '{_showRippleContainer}');
  })
  .css`
