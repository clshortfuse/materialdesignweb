...
function initializeMdwComponents() {
  ...
  for (var i = 0; i < components.length; i += 1) {
    componentMap.set(components[i], new _index.Menu(components[i]));
  }

  components = document.querySelectorAll('.js .mdw-menu__item');
  for (var _i = 0; _i < components.length; _i += 1) {
    componentMap.set(components[_i], new _index.MenuItem(components[_i]));
  }
}

/** @return {void} */
function setupJSButton() {
  var button = document.querySelector('.js .mdw-button');
  var menu = document.querySelector('.js .mdw-menu');
  button.addEventListener('click', function () {
    var mdwMenu = componentMap.get(menu);
    mdwMenu.show();
  });
}
