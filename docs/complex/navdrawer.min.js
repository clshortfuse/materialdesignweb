...
function configureNavDrawer() {
  ...
  var navdrawerDrawerRows = navdrawerDrawer.getElementsByClassName('mdw-list__row');
  ...
  function getSelectedRow(rows) {
    for (var _i = 0; _i < rows.length; _i += 1) {
      if (rows[_i].hasAttribute('selected')) {
        return rows[_i];
      }
    }
    return null;
  ...
  var _loop = function _loop(_i2) {
    var row = navdrawerDrawerRows[_i2];
    row.addEventListener('click', function () {
      if (row.hasAttribute('selected')) {
        return;
      }
      var currentlySelected = getSelectedRow(navdrawerDrawerRows);
  ...
  for (var _i2 = 0; _i2 < navdrawerDrawerRows.length; _i2 += 1) {
    _loop(_i2);
  }
}
