...
describe('Nested mdw-for deep/multi test (structure)', () => {
  /** @type {HTMLDivElement} */
  let container;
  ...
  it('renders nested mdw-for from a single `structure` observable and patches E', async () => {
    const DeepList = CustomElement
      .extend()
      .observe({
        // Use an object-style diff for nested arrays/indices inside `structure`
        // so the observer's `diff` produces per-index patches instead of
        // replacing arrays by reference (the default JSON Merge Patch behavior).
        structure: {
          type: 'object',
          // Provide typed defaults so the composition interpolation binds array
          // nodes (A..E) as arrays instead of `undefined` which prevents mdw-for
          // from constructing list nodes during initial interpolation.
          value: { A: [], B: [], C: [], D: [], E: [] },
          diff: (previous, current) => buildMergePatch(previous, current, 'object'),
        },
      })
      .html`
        <div>
          <div mdw-for="{a of structure.A}" class="A">
            <div class="a-title">{a.name}</div>
            <div mdw-for="{a1 of a.items}" class="A1">{a1}</div>
          </div>

          <div mdw-for="{b of structure.B}" class="B">
          ...
          <div mdw-for="{c of structure.C}" class="C">
            <div class="c-title">{c.name}</div>
            <div mdw-for="{c1 of c.items}" class="C1">{c1}</div>
          </div>

          <div mdw-for="{d of structure.D}" class="D">{d}</div>

          <div mdw-for="{e of structure.E}" class="E">
      ...
      `
      .register('deep-list-structure-test');

    container.innerHTML = '<deep-list-structure-test></deep-list-structure-test>';

    await customElements.whenDefined('deep-list-structure-test');

    /** @type {InstanceType<DeepList>} */
    const el = container.querySelector('deep-list-structure-test');

    // Set the whole structure via JSON Merge Patch semantics
    el.patch({
      structure: {
        A: [{ name: 'A', items: ['A1'] }],
        B: [{ name: 'B', items: [{ name: 'B1', children: ['B1a'] }] }],
        C: [{ name: 'C', items: ['C1'] }],
        D: ['D'],
        E: [{ name: 'E', items: [{ name: 'E1', children: ['E1a', 'E1b'] }] }],
      },
    });
    ...
    // Now perform a mutation: update E1 children (JSON Merge Patch) via `structure`
    el.patch({ structure: { E: { 0: { items: { 0: { children: { 1: 'E1b-updated' } } } } } } });
